package com.tekoi.game.constants;

public class ImagesPaths {

    //Menu
    public static final String MENU_BACKGROUND = "images/menu/menu_background.png";
    public static final String MENU_PLAY_BUTTON = "images/menu/menu_play.png";
    public static final String MENU_PLAY_BUTTON_PRESSED = "images/menu/menu_play_pressed.png";

    //Character
    //public static final String CHAR_ATLAS = "images/char/char_atlas_64x64.png";
    public static final String CHAR_ATLAS = "images/char/char_atlas_2_128x128.png";

    //Enemies
    public static final String BASIC_ENEMY = "images/enemies/basic_enemy_128x128.png";
    public static final String WALKING_ENEMY = "images/enemies/walking_enemy_128x128.png";

    private ImagesPaths() {}
}
